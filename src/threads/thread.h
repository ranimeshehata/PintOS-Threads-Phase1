...
struct thread
{
   ...
   /* ------------------------ ADDED ------------------------ */
   int real_priority;                 /* stores the real (original) priority of the thread without ant donation*/
   struct list locks_held;            /* the list occupies the locks held by the thread */
   struct lock *currentLock;          /* it points to the lock which the thread is currently waiting for */

   int nice;        /* how nice the thread should be to other threads*/
   real recent_cpu; /* recent cpu usage while thread is holding the CPU*/
